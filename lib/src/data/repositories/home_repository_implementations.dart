...
class HomeRepositoryImplementations extends HomeRepository {
  ...
  Future<DataState> createMaintenanceOffer({
    ...
    try {
      final httpResponse = await _homeApiServices.createMaintenanceOffer(
  ...
  @override
  Future<DataState<List<MaintainanceReports>>> getMaintenanceReports() async {
    try {
      final httpResponse = await _homeApiServices.maintenanceReports(
        GetTokenUseCase(injector())(),
      );
      if (((httpResponse.response.statusCode ?? 400) == 201) ||
          (httpResponse.response.statusCode ?? 400) == 200) {
